...
- `winrun/`: CLI runner.
  ...
  - `winrun -d <binary>`: debug mode (format, native capability, gdb function/argument trace for ELF targets, detected APIs/libraries, dispatch logs).
...
- `winrun -d` uses gdb tracing (`rbreak -> run -> frame/info args/backtrace -> continue`) for native ELF targets and records calls in execution order.
